...
public class SessieDTO implements Sessie {
	...
	public SessieDTO(String titel, String description, int aantalPlaatsen, String ITlabLokaal,
			String verantwoordelijke, LocalDateTime start, LocalDateTime einde, String gastSpreker,
			String state) {
		super();
		setTitel(titel);
	...
	public SessieDTO(ITlabSessie s) {
		...
		setITlabLokaal(s.getITlabLokaal().getLokaalcode());
		setVerantwoordelijke(s.getVerantwoordelijke().getGebruikersnaam());
		setStart(s.getStart());
		setEinde(s.getEinde());
		setGastSpreker(s.getGastspreker());
		setMedia(s.getMedia());
		setGeopendGeweest(s.isGeopendGeweest());
		this.setCurrentState(s.getCurrentState().toString());
	}
