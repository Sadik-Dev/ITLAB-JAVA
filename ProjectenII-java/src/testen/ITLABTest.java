...
class ITLABTest {
	...
	public void before() {
		...
		this.itlab = new ITLAB(mockSessieDao, mockGebruikersDao, mockLokaalDao, null);
		this.testITlabLokaal = new ITlabLokaal(LokaalType.Leslokaal, 60, "B1.025");
		...
		this.lokaallijst.add(new ITlabLokaal(LokaalType.Auditorium, 100, "B1.045"));
		...
		this.sessielijst.add(new ITlabSessie(	"azer",
												"zer",
												20,
												new ITlabLokaal(LokaalType.Auditorium, 30, "B1.036"),
												testVerantwoordelijke,
												LocalDateTime	.now()
																.plusDays(20),
												LocalDateTime	.now()
		...
		Mockito	.when(this.mockGebruikersDao.findAll())
				.thenReturn(new FilteredList<>(FXCollections.observableArrayList(gebruikerslijst)));

		Mockito	.when(this.mockLokaalDao.findAll())
				.thenReturn(new FilteredList<>(FXCollections.observableArrayList(lokaallijst)));

		Mockito	.when(this.mockSessieDao.findAll())
				.thenReturn(new FilteredList<>(FXCollections.observableArrayList(sessielijst)));
...
//	}
	...
	@Test
	public void wijzigSessie_sessieKanGewijzigdWorden_Succesvol() throws Exception {
		...
		Assertions.assertEquals(gewijzigdeSessieDTO, this.itlab.getSessie(gewijzigdeSessieDTO.getTitel()));
		Assertions.assertEquals(gewijzigdeSessieDTO.getAantalPlaatsen(),
			this.itlab	.getSessie(gewijzigdeSessieDTO.getTitel())
						.getAantalPlaatsen());
	}
	/*
	 * @Test public void wijzigSessie_registratieGesloten_throwsException() {
	 * SessieDTO dto = new SessieDTO(testSessie);
	 * 
	 * this.itlab.maakNieuweSessieAan(dto);
	 * 
	 * dto.setCurrentState(SessieStateEnum.RegistratieGeslotenAanmeldenOpenState.
	 * name());
	 * 
	 * assertThrows(IllegalArgumentException.class, () ->
	 * this.itlab.wijzigSessie(testSessie, dto)); }
	 * 
	 * @Test public void
	 * wijzigSessie_RegistratieEnAanmeldenGesloten_throwsException() { SessieDTO dto
	 * = new SessieDTO(testSessie);
	 * 
	 * this.itlab.maakNieuweSessieAan(dto);
	 * 
	 ...
	 * this.itlab.wijzigSessie(testSessie, dto)); }
	 * 
	 * // @Test // public void wijzigSessie_ongeldigeGegevens_throwsException() { //
	 * SessieDTO dto = new SessieDTO(testSessie); // //
	 * this.itlab.maakNieuweSessieAan(dto); // // SessieDTO gewijzigdeSessie = dto;
	 * // gewijzigdeSessie.setAantalPlaatsen(-50); // //
	 * assertThrows(IllegalArgumentException.class, () ->
	 * this.itlab.wijzigSessie(gewijzigdeSessie)); // }
	 * 
	 * // sessieoverzicht kunnen filteren
	 * 
	 * @Test public void filterSessie_geeftGefilterdeLijstTerug() {
	 * this.itlab.maakNieuweSessieAan(new SessieDTO(testSessie)); SessieDTO
	 * andereDto = new SessieDTO(testSessie); ITlabSessie andereSessie = new
	 * ITlabSessie( "azer", "zer", 20, new ITlabLokaal(LokaalType.Auditorium, 30,
	 * "qsdfqsd"), testVerantwoordelijke, LocalDateTime .now() .plusDays(20),
	 * LocalDateTime .now() .plusDays(20) .plusMinutes(60), "azerazer", null);
	 * 
	 * this.itlab.maakNieuweSessieAan(new SessieDTO(andereSessie));
	 * 
	 * // filteren itlab.filtreerSessies("ab");
	 * 
	 * // enkel de laatste sessie mag getoont worden assertEquals(1, this.itlab
	 * .geefSessies() .size());
	 * 
	 * }
	 */
